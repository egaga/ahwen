...
class LockTable {
     ...
     */
    private val locks = mutableMapOf<database.layer_0_file.Block, Int>()
     ...
     */
    fun acquireSharedLockTo(block: database.layer_0_file.Block) {
        lock.withLock {
            try {
     ...
     */
    fun acquireExclusiveLockTo(block: database.layer_0_file.Block) {
        lock.withLock {
            try {
                val startTime = Instant.now()
     ...
     */
    fun releaseLockOn(block: database.layer_0_file.Block) {
        lock.withLock {
            val value = getLockVal(block)
            if (value > 1)
                locks[block] = value - 1
    ...
    private fun hasExclusiveLock(block: database.layer_0_file.Block): Boolean =
        getLockVal(block) < 0

    private fun hasOtherSharedLocks(block: database.layer_0_file.Block): Boolean =
        getLockVal(block) > 1

    private fun getLockVal(block: database.layer_0_file.Block): Int =
        locks[block] ?: 0
