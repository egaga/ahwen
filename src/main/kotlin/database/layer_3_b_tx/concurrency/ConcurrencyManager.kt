...
class ConcurrencyManager(private val lockTable: LockTable) {
    ...
    private val locks = mutableMapOf<database.layer_0_file.Block, LockType>()

    fun acquireSharedLock(block: database.layer_0_file.Block) {
        if (block !in locks) {
    ...
    fun acquireExclusiveLock(block: database.layer_0_file.Block) {
