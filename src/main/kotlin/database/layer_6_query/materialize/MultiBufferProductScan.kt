...
import database.layer_4_record.RecordPage
import database.layer_4_record.TableInfo
import database.layer_6_query.ProductScan
import database.layer_6_query.Scan
import database.layer_6_query.SqlValue
import database.layer_6_query.bestFactor
...
class MultiBufferProductScan(
    ...
    private class ChunkScan(
        ...
        private val pages: List<RecordPage> = (startBlockNum..endBlockNum).map { blockNum ->
            RecordPage(database.layer_0_file.Block(ti.fileName, blockNum), ti, tx)
        }
